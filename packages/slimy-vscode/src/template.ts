...
export function template(variant: SchemeName, italics: boolean): ColorTheme {
	...
	const inputForeground = scheme.base.fg;
	const inputBackground = scheme.ui.popover.bg;
	const inputSelectionBackground = prefers(
		inputForeground,
		scheme.base.accent,
		scheme.base.accent[darkenIfDark(type)](0.2),
		scheme.base.accent[darkenIfDark(type)](0.4),
		scheme.base.accent[darkenIfDark(type)](0.6),
		scheme.base.accent[darkenIfDark(type)](0.8)
	);
	...
	return {
		...
		colors: {
			...
			// Background color of text selections in the workbench (for input
			// fields or text areas, does not apply to selections within the
			// editor and the terminal)
			"selection.background": inputSelectionBackground.alpha(0.5).hex(),
			...
			"input.background": inputBackground.hex(),
			"input.border": scheme.ui.popover.border?.hex() || transparent,
			"input.foreground": inputForeground.hex(),
